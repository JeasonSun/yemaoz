...
import { ParameterException } from '~/plugin/http-exception'
import { get, last, set, cloneDeep } from 'lodash'
import { findMembers } from '~/plugin/utils'
...
class LinValidator {
  ...
  async _check (key, alias = {}) {
    ...
    if (isCustomFunc) {
      try {
        const checkResult = await this[key](this.data)
        // 如果返回数据了， 那么
        if (checkResult === undefined || checkResult === true) {
          result = new RuleResult(true)
        } else {
          // 如果是数组，直接返回错误数组[false, msg]
          if (Array.isArray(checkResult)) {
            result = new RuleResult(...checkResult)
          } else {
            // 如果是字符串，返回[false, checkResult]
            result = new RuleResult(false, checkResult)
          }
        }
      } catch (error) {
...
class Rule {
  ...
  validate (field) {
    ...
    if (this.name === 'isNotEmpty') {
      const isEmpty = validator.isEmpty(field)
      if (isEmpty) {
        return new RuleResult(
          false,
          this.msg || this.message || `${this.name}参数不能为空`
        )
      } else {
    ...
    if (!validator[this.name]) {
      return new RuleResult(false, `服务器错误，不存在该规则:${this.name}`)
    }
